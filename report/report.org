...
#+attr_latex: :width 16.0cm :options angle=0
...
The processor is coupled to the HPS through the lightweight HPS-to-FPGA AXI bridge. In the implemented system, the bridge base address is /0xFF200000/ and the software utilities use a mapped span of /0x00200000/. This interface allows the HPS to access the processor memories and exported status registers directly from Linux user space through /open("/dev/mem")/ and /mmap()/.
...
The decoder currently distinguishes five opcode classes: R-type, /lw/, /sw/, /beq/, and /addi/. For example, the R-type opcode /000000/ activates /RegDst/, /RegWrite/, and the R-type /ALUOp/ code, while /lw/ activates /ALUSrc/, /MemtoReg/, /RegWrite/, and /MemRead/. The most specific implementation detail is the handling of the write enable for data memory. Rather than exposing a single-bit /MemWrite/, the design expands it into a /4-bit/ byte-enable bus. Whenever a store instruction is detected, the BRAM enable process drives /1111/, allowing a full /32-bit/ word write. This choice matches the byte-enable interface of the Platform Designer on-chip memory block.
