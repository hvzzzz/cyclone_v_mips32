...
\begin{figure}[htbp]
\centering
\includegraphics[angle=0,width=17.0cm]{./images/block_diagram.png}
\caption{\label{fig:general_schema}General Block Diagram}
\end{figure}
\section{Project specification}
\label{sec:orgb7f9703}
...
\label{sec:org2740a34}

The project followed a bottom-up roadmap consistent with the milestones defined during the project follow-up meeting. This was a solo project completed over a \emph{10-week} period from \emph{March 20} to \emph{May 25, 2026}. The work started with HPS-FPGA communication through the AXI bridge, continued with the design and simulation of the elementary processor blocks, then moved to datapath and memory integration, and finally reached board-level validation with assembly programs, LCD monitoring, and HPS-side debug tools. From a project-management perspective, this progression can be summarized in four phases: specification and component design, datapath integration, memory and board implementation, and final validation.
...
\label{sec:org9d9a5bc}

Table \ref{tab:timeline} summarizes the major milestones and development phases throughout the project duration, as recorded in the version control history. The project spanned \emph{10 weeks} and progressed through four distinct phases.

\begin{table}[htbp]
\caption{\label{tab:timeline}Project timeline and major milestones}
\centering
\begin{tabular}{lll}
\hline
Date & Phase & Key Accomplishments\\
\hline
\textbf{March 20} & \textbf{Phase 1: Infrastructure Setup} & • HPS-FPGA AXI bridge validated\\
(Week 1) &  & • Memory-mapped read/write confirmed\\
 &  & • Build automation (Makefiles) added\\
\hline
\textbf{March 27} & \textbf{Phase 1: Component Design} & • ALU designed and simulated\\
(Week 2) &  & • ALU control unit implemented\\
 &  & • Project presentation prepared\\
\hline
\textbf{April 3} & \textbf{Phase 1: Core Components} & • Register file (32×32-bit) completed\\
(Week 3) &  & • Program counter with enable/reset\\
 &  & • Unit testbenches validated\\
\hline
\textbf{April 10} & \textbf{Phase 2: Memory Integration} & • BRAM blocks instantiated\\
(Week 4) &  & • PC connected to instruction memory\\
 &  & • Main control decoder added\\
\hline
\textbf{April 17} & \textbf{Phase 2: Datapath Assembly} & • Complete datapath integrated\\
(Week 5) &  & • All processor blocks wired\\
 &  & • Top-level structural design done\\
\hline
\textbf{April 24} & \textbf{Phase 3: First Working System} & • Processor executes on FPGA\\
(Week 6) &  & • Initial instruction tests passed\\
 &  & • Hardware functionality confirmed\\
\hline
\textbf{May 2} & \textbf{Phase 3: Execution Control} & • Manual step-by-step mode added\\
(Week 7) &  & • Free-run continuous mode added\\
 &  & • Switcher block with stage control\\
\hline
\textbf{May 4} & \textbf{Phase 4: Demonstration Programs} & • Fibonacci sequence validated\\
(Week 8) &  & • LCD monitor fully integrated\\
 &  & • HPS observation tools operational\\
\hline
\textbf{May 15} & \textbf{Phase 4: Final Validation} & • ISA test program developed\\
(Week 9) &  & • Additional assembly tests added\\
 &  & • Pin assignments optimized\\
\hline
\end{tabular}
\end{table}
\subsection{Development toolchain and simulation infrastructure}
\label{sec:orga99c575}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=11cm]{./images/register.png}
...
The decoder currently distinguishes five opcode classes: R-type, \emph{lw}, \emph{sw}, \emph{beq}, and \emph{addi}. The specific opcode assignments are summarized in table \ref{tab:opcodes}.

\begin{table}[htbp]
\caption{\label{tab:opcodes}Opcode assignments for the implemented instruction subset}
\centering
\begin{tabular}{lrr}
\hline
...
The \emph{addi} instruction represents an extension beyond the reference single-cycle processor described in the Hennessy and Patterson textbook, which originally included only \emph{add, sub, and, or, slt, lw, sw,} and \emph{beq}. The addition of \emph{addi} with opcode \emph{001000} (0x08) improves the processor's practical utility by allowing immediate-value arithmetic without requiring a separate load operation, which is particularly useful for loop counters, address calculations, and constant initialization in programs like the Fibonacci demonstration.

For each opcode, the control decoder generates a \emph{9-bit} intermediate vector that is then mapped to individual control signals. For example, the R-type opcode \emph{000000} produces the vector \emph{100100010}, which activates \emph{RegDst}, \emph{RegWrite}, and sets \emph{ALUOp} to \emph{10} for R-type decoding. The \emph{lw} instruction (opcode \emph{100011}) produces \emph{011110000}, which activates \emph{ALUSrc}, \emph{MemtoReg}, \emph{RegWrite}, and \emph{MemRead}. The \emph{addi} instruction (opcode \emph{001000}) generates \emph{010100000}, which activates \emph{ALUSrc} and \emph{RegWrite}, allowing it to reuse the ALU addition path with the sign-extended immediate value as the second operand.

The most specific implementation detail is the handling of the write enable for data memory. Rather than exposing a single-bit \emph{MemWrite}, the design expands it into a \emph{4-bit} byte-enable bus. Whenever a store instruction is detected, the BRAM enable process drives \emph{1111}, allowing a full \emph{32-bit} word write. This choice matches the byte-enable interface of the Platform Designer on-chip memory block.
\subsection{ALU and ALU control implementation}
\label{sec:orgf699399}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=8cm]{./images/table_alu.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=9cm]{./images/bram.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./images/platform_designer.png}
...
\label{sec:org61e2172}
\subsection{Functional validation}
\label{sec:org949a2f6}
